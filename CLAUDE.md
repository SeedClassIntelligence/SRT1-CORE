...
1. Do not introduce private Enterprise implementation into Core.
2. Do not import, expose, or recreate private Seed Signature authority, private keys, SCIA memory implementation, SCIA security implementation, SION internals, private audit chain, or Enterprise backend.
...
4. FileCell is a local containment concept, not Enterprise-only. Treat FileCell, manifest derivation, verification, and operational registry as Core/Pro candidates only when decoupled from SION, private signing, and private ledger implementation.
5. SRT-1 Core observes, prepares, constrains, and verifies. It does not autonomously merge, remediate, or execute code changes.
6. Enterprise/private integrations must fail closed when unavailable.
